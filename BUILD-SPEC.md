...
**Phase 5 outcome.** Built `lib/gates.ts` (real Supabase count queries per placement, matching this table exactly — including the call-sheet staleness rule and the Set Life 100 admin-enable flag, backed by a new `site_settings` migration) and an admin gate-status page at `/admin/gates`, not yet behind auth (§45 wraps every `/admin/*` route in one pass in Phase 9; this page is metadata, not editorial content, so the temporary gap is deliberately small). Verified against `supabase/seed.sql`'s real, deterministic counts — both branches of the met/unmet boolean and the shortfall arithmetic are exercised by existing seed data (current issue/shop clear their minimum; every placement-driven gate falls short of it), not asserted in the abstract. **This VERIFY block's homepage-composition half — "assert the homepage renders cleanly with only Hero, Newsletter, and Footer... raise one section above threshold, assert correct sequence position" — cannot be exercised yet.** §22-38 don't exist as components to compose until Phase 6-8, and §37 Newsletter is Phase 8 scope. Re-run this VERIFY in full at the end of Phase 8, once every section actually consumes `lib/gates.ts`'s thresholds to self-gate.

**Real infrastructure bug found and fixed, unrelated to §9 itself but found while building it.** A from-scratch `supabase start` denied every anon/authenticated/service_role query against every table with "permission denied for table X" — despite correct RLS policies existing since Phase 2. RLS only ever narrows access the underlying Postgres `GRANT` already allows; it was never a substitute for one, and no migration here had ever issued one, relying on implicit role bootstrapping that isn't guaranteed across Postgres/PostgREST image versions. This had been silently masked until now (Phase 4's CI fix gave every PR a fresh local Supabase instance for the first time, and this project's own long-running local dev database had drifted into a state where it happened to work, obscuring that migrations alone don't reproduce it). Fixed with an explicit grants migration (`20260812185623_grant_table_privileges.sql`) — the same baseline every hosted Supabase project ships with by default, now made explicit instead of assumed.
...
**§16.1 Mobile menu backdrop — confirmed bug, required fix.** Real-device audit (390×844 viewport) found the full-screen mobile menu's opaque `--black` backdrop does not reliably cover the full scrollable height of the menu's link list — several editorial/utility links render floating over the hero video with no background beneath them, instead of over solid black. This is a mobile-only defect; the desktop/tablet mega-menu and scrolled-state nav are unaffected and must not be touched while fixing this. **Required fix:** the mobile full-screen menu's backdrop element must be sized to `100vw × 100dvh` as a fixed/absolute-positioned layer *independent of its content's height* (i.e., the backdrop is pinned to the viewport, not sized to fit however tall the link list happens to render) — every link must sit visually on solid `--black`, with zero exceptions, at every scroll position within the open menu, at 375px, 390px, and 428px widths. Confirm this by opening the menu and checking there is no point, at any scroll position inside the open menu, where the video or page content is visible behind a link. This fix is scoped strictly to the mobile (`< 768px` or existing mobile breakpoint) full-screen menu markup/CSS in `GlobalNav.tsx` — it must not alter desktop nav markup, desktop mega-menu, or any desktop-breakpoint styles.

**VERIFY §16.1** — At 390×844: open the mobile menu, screenshot with menu at rest (no scroll) — solid black fills 100% of the viewport behind every visible link. If the link list is taller than the viewport and scrolls, screenshot again scrolled to the bottom of the menu — still solid black behind every link, no video/page bleed-through anywhere. Confirm desktop (1440×900) nav and mega-menu are pixel-identical to their pre-fix state — this is a mobile-only fix and must produce zero desktop diff.

**§16.2 Desktop/tablet nav — "Industry" label sits lower than its siblings. Confirmed bug, required fix.** Owner-reported and reproduced at 1440×900 on both the homepage (scrolled/solid state) and an interior page's permanent solid nav (`/about`) — same shared `GlobalNav` component, so this is a component-level defect, not page-specific. Every nav label — Magazine, Film & TV, Spotlights, Reviews, Watch, Shop — sits on one shared baseline; "Industry," the mega-menu trigger, renders visibly lower than the rest, breaking the row's alignment. Likely cause: Industry is the one label implemented as a `<button>` rather than an `<a>`, and the button element's default line-height/vertical-align or padding differs from the surrounding links' — confirm by inspecting computed style, not by guessing further. **Required fix:** the Industry trigger's text must sit on the exact same baseline as every other nav label, at every breakpoint where the full label row is visible (desktop and tablet — this is not a mobile-menu issue, mobile uses the full-screen menu covered by §16.1). Fix by normalizing the button's line-height/vertical-align/padding to match the sibling `<a>` elements — do not change any other nav element's markup or styling to compensate.

**VERIFY §16.2** — At 1440×900 and a representative tablet width (e.g. 834px), on both the homepage (scrolled/solid state) and an interior page: screenshot the nav row, confirm all nav labels including Industry share one visual baseline with zero vertical offset. Confirm the mega menu itself (open/close, focus trap, Escape behavior) is unchanged — this fix touches only the trigger's own vertical alignment, not its function.

## §17 Hero — Full Viewport

**The highest-priority surface in this specification.** This section describes desktop and mobile-landscape. **Mobile portrait has its own, deliberately different treatment — §42.1 — that overrides the full-bleed rule below. Read §42.1 before assuming this section is absolute on mobile.**
...
**Phase 8 outcome — §35-§38, closing out the homepage build (§22-§38 complete).** §35 reuses `ProductCard` (built Phase 2, already single-component/no-branching per its own docstring) — seeded one of each product shape to actually exercise that claim rather than trust it. §36 has no `INSTAGRAM_ACCESS_TOKEN` in any environment this build runs in, so it always takes the CMS fallback path today — real, working degradation, not a placeholder, built from already-published editorial images rather than a new curation surface. §37 is real persistence (`newsletter_subscribers` table + a Server Action), not a UI mock; "post-signup preferences" is a genuine second step, not inline with the initial form. §38 rebuilt `SiteFooter.tsx` entirely: Explore/Industry columns now derive from `nav-data.ts`'s `PRIMARY_NAV` (§16's own single source of truth) instead of a second hand-maintained list, and the five Legal routes exist with placeholder-honest content — each one explicitly flagged in its own page as unreviewed, per this section's own instruction not to ship AI-authored legal language silently.

**Real bugs found and fixed, both the same underlying class of Postgres/PostgREST behavior, neither obvious from reading the code:**
- **`UPDATE ... WHERE id = $1` requires `SELECT` privilege to evaluate the `WHERE` clause, independent of `UPDATE` being granted.** The preferences-save action returned `{ok: true}` with no error while silently updating zero rows — anon has `UPDATE` on `newsletter_subscribers` but deliberately no `SELECT` (to prevent enumerating subscriber emails), and Postgres needs `SELECT` just to *find* the row before it can update it. Confirmed directly via a bare `curl PATCH` reproducing the exact "permission denied" the ORM was swallowing. Fixed by moving that one write to a new service-role client (`lib/supabase/service.ts`, server-action-only) instead of granting anon the `SELECT` that would have defeated the whole point of restricting it.
- **The Phase 5 grants migration's `grant all on all tables ... to service_role` is a snapshot, not a standing default.** `newsletter_subscribers` (created after it) started with zero service-role privileges — same class of bug as Phase 5's original finding, now confirmed to recur for every table added afterward unless its own migration grants service_role explicitly. Every future migration that creates a table needs to do this itself; noted directly in the newsletter migration's own comment as a reminder for Phase 9+.

Also found and fixed, pre-existing and unrelated to Phase 8's new work: the footer already had two `href="#"` placeholders (Facebook/TikTok icons) and a dead "Newsletter" `href="#"` link, directly violating §38's own "zero `href='#'` placeholders at any phase" and "only active channels" rules — removed along with the rebuild, not left in place.
...
- **Hero — orientation-dependent, owner-directed change from full-bleed (see below).**
...
### §42.1 Mobile Hero — Owner-Directed Override of §17's Full-Bleed Rule

**Found during a live mobile audit, not a hypothetical.** At real mobile widths, forcing the 16:9 hero footage to fill a `100dvh` portrait container crops the composition badly — the center subject's face is cut off at the nav line, side subjects are nearly cropped out entirely. §42's original instruction was to art-direct `object-position` per-subject to compensate. **The owner overrode this instead of patching around it:** stop cropping the video on mobile at all.

**This intentionally reverses §17's "full-bleed, edge to edge, no margin, no light strip beneath the video" rule and §43's "no pixel of §22 visible before first scroll" assertion — but only for mobile portrait. Desktop's full-bleed cinematic treatment is unchanged.** Do not read this as license to soften §17 anywhere else; it is a scoped, deliberate exception, made explicitly, for exactly this one context.

**Portrait (the default mobile case):**
- Navigation is a normal, solid `--black` bar in document flow at the top — 72px, matching §16's existing mobile nav — **not** transparently overlaid on the video. There's no full-bleed background left to overlay on top of.
- The hero video renders directly below the nav bar, full viewport width, at its **native 16:9 ratio** (`height = width × 9/16` — roughly 220px tall at 390px width). Letterboxed, not cropped. The entire frame is always visible; no `object-position` compensation is needed because nothing is cut off.
- Page content (§17.1, then §22) begins immediately below the video. On most phones this means part of §17.1 — and sometimes the top of §22 — is visible without scrolling. **This is expected and correct under this override**, not a regression of §17/§43's "nothing visible before first scroll" rule, which no longer applies in this orientation.
- Video controls stay bottom-right, positioned within the now-smaller video frame, not the full viewport.

**Landscape (phone rotated):** revert to the desktop-style full-bleed treatment — video fills `100svh`/`100dvh`, nav returns to a transparent overlay on top of it. Landscape phone dimensions are close enough to 16:9 that full-bleed cropping is minor, the same tradeoff desktop already makes without issue. **This is a CSS layout switch on the same video element, keyed off `(orientation: landscape)`** — not the browser Fullscreen API, not an actual player-chrome mode change like a native video app. No JS rotation-lock or fullscreen-request logic is needed; it's the existing responsive system with one more state.

**Escalate, do not decide unilaterally:** whether the ~40 MB hero video is acceptable over cellular, or whether a lighter mobile encode via `<source media="...">` is warranted (§6.1 already specs this file; confirm it's actually been produced).

**VERIFY §42.1** — At 390×844 and 414×896 (portrait): assert the video element's rendered height equals its width × 9/16 (± rounding), assert zero cropping (compare rendered frame against source aspect ratio, not just "looks right"), assert the nav bar has an opaque background with `position: static` or equivalent (not `fixed`/`absolute` overlay), assert no content is clipped between video and §17.1. At the same devices rotated to landscape: assert the video reverts to full-bleed `100svh`/`100dvh` and nav reverts to a transparent overlay. Assert the transition when rotating mid-scroll doesn't leave the page in a broken intermediate state (e.g., re-check scroll position and layout after orientation change fires).
...
At **1440 × 900** (desktop), screenshot immediately after load.
...
**At 390 × 844 (mobile portrait) — different gate, per §42.1.** Full-bleed does not apply here by design. Must contain: solid opaque nav bar at top · the hero video at native 16:9 ratio directly beneath it, uncropped · video controls within the video frame. Must not contain: any cropped or cut-off subject framing · a transparent nav overlay (there's no full-bleed background to overlay). §17/§43's original "nothing below the video visible before scroll" does not apply in this orientation — see §42.1 for why. **At 844 × 390 (mobile landscape)** the original desktop-style gate above applies — full-bleed, transparent nav overlay, nothing cropped beyond what desktop already accepts.
...
**Phase 9 outcome — §45.** Built on branch `feat/admin-cms`: Supabase Auth login (route protection added to `lib/supabase/middleware.ts`, redirecting any unauthenticated `/admin/*` request to `/admin/login`), the section-tile dashboard, the full Tiptap article editor (`components/admin/{TiptapEditor,PostEditor,MetaFieldsPanel,HeroImageUpload}.tsx`) with drag-and-drop image upload direct to Supabase Storage, and CRUD for every entity the route table lists — issues, products, media library, authors, honorees, and a new `submissions` table + queue backing `/admin/submissions` (§48.1 notes submissions "land in the admin queue as unpublished" — built now since `/submit`'s form had zero backend before this phase; full spam/rate-limit defenses from §48.1 itself are still open, deferred to the phase that builds Stripe/commerce hardening). `/submit` now writes real rows via a server action instead of `DemoForm`'s client-only fake "Sent" state.

Two real bugs found and fixed, both would have silently broken publishing for every future contributor:

1. **Publish-on-a-new-post silently no-opped.** `handlePublish` read `postId` from a stale closure after `await handleSave()` — `setPostId` inside `handleSave` doesn't mutate the closure's already-captured `postId`, so `if (!postId) return` fired every time on a brand-new post's first Publish click, after the draft save had already succeeded. Fixed by having `handleSave` return the id directly and using that returned value instead of reading state.
2. **A second, subtler version of the same class of bug, found only via Playwright, not code review:** even after fix #1, `handlePublish` still hung indefinitely on "Publishing…" for a new post. Root cause: the save step's `router.replace()` (correctly) navigates from `/admin/posts/new` to `/admin/posts/[id]`, and that navigation aborts the rest of the *same* event handler's execution — `publishPost()` was called immediately after `handleSave()` returned, i.e. after the navigation had already been kicked off, and never ran. Fixed by extracting a `savePostRaw()` helper with no state writes or navigation, so `handlePublish` can run save-then-publish fully to completion and only navigate once, at the very end.

**Real, non-obvious Next.js 16 Server Action finding:** passing the Tiptap body (`{type:"doc", content:[...]}`) directly as a nested-object Server Action argument silently drops nested `attrs` sub-objects — confirmed by three-point instrumentation (browser `onUpdate` log showed the image node's `attrs.src` present; a `console.log` inside the Server Action itself, read from the `next start` process's own stdout, showed `{"type":"image"}` with `attrs` gone entirely; the raw multipart network payload contained a `"$T"` Flight chunk-reference token for that field, meaning the browser *sent* it correctly and something in the framework's argument decoding lost it). Workaround, not a real fix (root cause is inside Next.js's Server Action argument serialization, not this app's code): `JSON.stringify()` the body on the client and send it as a plain string (`SavePostInput.bodyJson`), `JSON.parse()` it first thing inside `savePost`. Plain strings round-trip correctly; nested objects with this specific shape do not. Worth re-testing without the string workaround on a future Next.js upgrade.

**Editor-interaction finding (Playwright-specific, not an app bug):** typing text and *then* toggling a block to a heading works reliably; toggling an *empty* block to a heading *before* typing into it does not — StarterKit's `TrailingNode` extension (bundled by default in Tiptap 3.x, keeps an editable paragraph after headings/images) relocates the cursor into that auto-inserted trailing node, so subsequent keystrokes land in the wrong place. `tests/e2e/admin.spec.ts` types all blocks as plain paragraphs first, then clicks back into each one by its text to convert it — the reliable sequence, confirmed by direct DOM inspection.

**Known, intentional scope split from VERIFY §45's text, not a silent gap:** the "confirm all four render correctly on the public page" half of the spotlight assertion is deferred to Phase 10 (§46) — `/story/[slug]`'s renderer is still the minimal one this phase upgraded just enough to walk Tiptap JSON in reading order (`app/story/[slug]/page.tsx`'s `PostBody`/`TiptapNode`, replacing the old array-only renderer that predates Tiptap and would have silently rendered nothing for any post saved through the new editor). `role_line`, the credits list, the pull quote, and platform badges all save and round-trip correctly (asserted in `admin.spec.ts` by reopening the edit page after save), but none of the four render on the public page yet — that's §46's magazine-format rendering system, not built until Phase 10. Flagging this now rather than letting Phase 10 discover it as a surprise.

`tests/e2e/admin.spec.ts`'s full-workflow test runs desktop-only (`test.skip` on the mobile project) — it isn't testing responsive layout, and running the same write-heavy sequence from both viewport projects concurrently against the one shared local Supabase instance proved racy, the same class of flakiness `gates.spec.ts`'s own comment already documented for read assertions. The two access-control tests (logged-out `/admin` and `/admin/posts` redirect checks) still run on both, since they're cheap and read-only.

The one shared login credential is provisioned via `supabase/seed.sql` (local/CI only, same as every other seed row — never runs against the real project) rather than through any interactive sign-up flow, since generating and inserting a test credential for an ephemeral local database is a standard test-fixture pattern, not the same thing as handling a real person's password.

**§45.1 Post editor — Details box breaks its panel margins. Confirmed bug, not yet fixed.** Owner audit of the live production admin CMS (post editor, right-side metadata panel) found the "Details" box — the category-specific fields block (e.g. News Category on a News-category post) — does not stay within the panel's inner margins the way every other box in that column does. The Hero Image, Details, and SEO boxes should all sit flush inside the panel's padding with consistent left/right insets; instead the Details box's border/content extends past that panel edge, breaking the column's alignment. This is visible at desktop width (1440px) and has not yet been checked at mobile widths.

**Required fix:** the Details box must respect the same container width and horizontal padding as the Hero Image box above it and the SEO box below it — likely a missing `max-width: 100%` / box-sizing fix, or a hardcoded width on the Details block's wrapper that isn't accounting for the panel's padding. Scope this to the Details block's own styling — do not change Hero Image or SEO box styling, which are already correct.

**VERIFY §45.1** — At 1440×900: open the post editor for a News-category post (or any category with a category-specific Details block). Screenshot the right-side panel. All three boxes — Hero Image, Details, SEO — must have identical left and right edges, flush with the panel's inner padding, with no box's border or content extending past the panel boundary. Repeat for every post category that renders a category-specific Details block (News, Production listing, Review, Video, Opportunity, Festival — per §44's conditional fields) since this may be a shared component used across all of them, not a News-only bug.

**Known coverage gap, not yet closed:** `tests/e2e/admin.spec.ts`'s full admin workflow test runs desktop-only by design (`test.skip` on the mobile Playwright project) — per the Phase 9 outcome note above, Phase 9 shipped with zero automated mobile/responsive coverage on the editor screens. §42's admin mobile requirement (strengthened to phone width, not just tablet, per the owner-directed mobile remediation pass) has not been verified on any real device or viewport yet. This is not a regression Phase 9 introduced — it's an explicit, documented scope split — but it means the admin CMS's mobile behavior is still an open item, not a closed one, despite Phase 9 being merged to production.

**§45.1 — first fix attempt (commit `9bd416e`) landed, confirmed still broken, root cause corrected.** That commit added `width: 100%; box-sizing: border-box` to `.admin-meta-panel`. Owner re-audit of the live page after deploy confirms the Details box still breaks the panel margin — the fix did not hold. Root cause was misdiagnosed: `.admin-meta-panel` is a `<fieldset>` (confirmed by the adjacent `.admin-meta-panel legend` rule — only `<fieldset>` can have a `<legend>` child), and `<fieldset>` carries a browser-default intrinsic `min-width` sized to its own unshrinkable content, not `0`. Inside a `display: flex; flex-direction: column` parent, that intrinsic `min-width` overrides `width: 100%` — the box will still refuse to shrink below its content's natural width. The Details fieldset's category-label text (e.g. `NEWS CATEGORY (CASTING/DEVELOPMENT/IN PRODUCTION/ACQUISITION/DISTRIBUTION/AWARDS/STREAMING/BUSINESS)`) is exactly the kind of long, slash-delimited, hard-to-wrap string that sets a wide min-content width — which is almost certainly why the Details box specifically breaks out while the shorter-labeled SEO fieldset doesn't.

**Corrected required fix:** add `min-width: 0;` to `.admin-meta-panel` alongside the `width: 100%; box-sizing: border-box` already there — `width: 100%` alone is not sufficient on a flex-child `<fieldset>` and should not be relied on in isolation. Additionally add `overflow-wrap: break-word;` (or `word-break: break-word;`) to the category-label text itself, since even with the width constrained, a long unbreakable label could still visually overflow its own line rather than wrap. Verify against the specific long label above, not just a short one — a short label passing is not sufficient evidence this is fixed.

**VERIFY §45.1 (re-run required)** — Same as above, but explicitly test against a post using the longest known category label (News category's parenthetical list) at 1440×900, not just any category-specific field. The prior fix passed CI's existing checks (2/3, not 3/3 — the third check's failure has not yet been triaged and may or may not be related; check before assuming it's unrelated) but did not actually resolve the visual bug, which is a signal that no automated test currently asserts this box's boundary — consider adding one rather than relying on manual screenshot review alone going forward.

**§45.1 — corrected fix verified, one more real bug found in the process, CI question answered.** Commit `bf19358` (`min-width: 0` + `overflow-wrap: break-word`, exactly as prescribed above) existed locally but had never been pushed — nothing had actually re-run CI or been re-audited against it yet. Verified before landing:

- **Overflow itself:** confirmed fixed. A Playwright check against `/admin/posts/new?category=news` at 1440×900 (News's parenthetical label is the longest known — deliberately not testing a short-labeled category) measured `document.documentElement.scrollWidth === clientWidth` (1440 = 1440, zero page-level overflow) and screenshotted the panel: the News Category label wraps cleanly across three lines inside its box, no clipping, no bleed.
- **A second, related bug found by the same check, not covered by the prescribed fix:** Hero Image, Details, and SEO did not actually share identical left/right edges — Details/SEO sat 2px right of Hero Image. Root cause: `<fieldset>` carries a browser-default `margin` (`0 2px` in Chrome's UA stylesheet) that `.admin-meta-panel` never reset, so the two fieldsets (Details, SEO) sat indented relative to their non-fieldset sibling (`.admin-hero-upload`, a plain `<div>`). Fixed by adding `margin: 0` to `.admin-meta-panel` alongside the existing `min-width: 0`. Re-measured after the fix: all three boxes' `x` and `x + width` match exactly (0px difference, not just within a rounding tolerance).
- **The "don't assume it's unrelated" CI question:** checked directly via the GitHub API rather than assumed. Commit `9bd416e`'s failing check was 6 failing tests, none about the Details panel — they were `unexpected text nodes directly in the hero` / Master Gate and home visual-baseline screenshot mismatches (expected fallout of that same commit's §42.1 mobile hero redesign changing the mobile hero from full-bleed video to a letterboxed, static-nav layout — the linux baseline PNGs hadn't been regenerated yet) plus one `admin.spec.ts` full-workflow timeout. Confirmed genuinely unrelated to §45.1's CSS, not assumed. Separately, the *current* remote `main` tip (`7711fd1`) also has a failing push-triggered CI run for the same two visual-baseline reasons — this is a real, still-open item (mobile hero/home linux snapshots are stale after the §42.1 redesign) but out of scope for this note; flagging it here so it isn't mistaken for something §45.1 caused.
- **Automated coverage added**, closing the gap the prior note flagged: `tests/e2e/admin.spec.ts` now has a permanent `§45.1 admin editor sidebar — fieldset overflow regression` test asserting the three boxes' edges match and the page has zero horizontal overflow, run against the News category specifically (the case that actually exercises the bug). Runs on both viewport projects since it does no writes.
...
**Phase 10 outcome — §46.** Built the full magazine reading experience at `app/story/[slug]/page.tsx`, replacing the Phase 9 placeholder renderer: animated on-load headline reveal (`@keyframes article-reveal`, staggered across eyebrow/role-line/headline/dek/byline, respects `prefers-reduced-motion`), oversized Anton section headers using the §12 type-scale tokens for the first time anywhere in the codebase (`--type-editorial-h1`, `--type-feature-headline` — previously defined in Phase 3 but never actually applied, per that phase's own outcome note), pull quotes at a clamped 28-38px, a sticky scroll-tracked reading-progress bar (`components/story/ReadingProgress.tsx`), share actions via the native Web Share API with a copy-link fallback, and related content (`lib/queries.ts`'s new `getRelatedPosts` — same category, then same author, then most recent, backfilling each tier only as far as needed — and `getNextArticle`).

**A real Phase 9 gap closed, not deferred further:** §45 explicitly requires a callout/feature box, "reusable across any article, not spotlight-specific" — Phase 9 built role_line, credits, and platform badges but never actually built this one, and nothing caught it since no VERIFY step exercised it in isolation. Added `components/admin/CalloutField.tsx` as a universal sidebar field (not gated by `lib/admin-meta-fields.ts`'s per-category system, since a callout is meant to work on any category), storing `meta.callout = {heading, icon?, items}`. `components/story/CalloutBox.tsx` renders it with the same zero-render discipline as credits/badges.

**Still-standing, disclosed scope simplification (carried over from the Phase 9 note, not resolved here):** credits list, platform badges, and the callout box remain separate `meta` fields rather than custom Tiptap node types, so they render in a fixed position after the body rather than truly interleaved wherever the contributor placed them in the block sequence, as this section's text literally describes. Rebuilding them as first-class ProseMirror NodeViews is a materially larger lift than either phase scoped. Similarly, "full-width and inset image blocks" is implemented as a single mode — images render at the article body's own measure (760px), not full viewport-bleed — since the editor has no per-image toggle to choose between the two; a real distinction would need one added to `components/admin/TiptapEditor.tsx`'s image node.

**Real bug found and fixed while building this:** `readingTimeFrom()` (used for the post's `reading_time` column) computed word count from `JSON.stringify(body).split(/\s+/).length` — literally counting JSON syntax tokens (`"type":`, `"paragraph",`) as words, not the article's actual prose. This would have made every reading-time estimate meaningfully wrong, not just imprecise, and would never have been caught by eyeballing the admin UI since the number always looked plausible. Replaced with `lib/tiptap-text.ts`'s `extractPlainText()`, which walks the Tiptap doc and concatenates only real `text` node content.

**Verification:** built a full test article through the real admin editor (two section headers, a pull quote, an uploaded image, `role_line`, a 3-entry credits list, 2 platform badges, and a callout box with 2 bullets) and screenshotted the rendered result at 1440×900 and 390×844 — both render cleanly, no overflow, credits/badges/callout all present in the sidebar-adjacent-but-in-body position described above. Confirmed a non-spotlight post shows zero trace of any of these blocks. Confirmed the reading-progress bar goes from 0 to exactly 100 across a real scroll on a tall page (a short seed post that fits entirely within the viewport correctly starts at 100 immediately instead — "nothing left to scroll" is a legitimate state, not a bug — `tests/e2e/story.spec.ts` asserts the bottom-of-page invariant rather than assuming a universal starting value). Added `tests/e2e/story.spec.ts` as permanent regression coverage for both this section and §47 below, since neither had any before this phase.
...
**Phase 11 outcome — §47.** `lib/structured-data.ts` builds every schema in the table above; `components/JsonLd.tsx` renders each as a `<script type="application/ld+json">`, escaping `<` so a CMS-sourced value containing `</script>` can't break out of the tag. `Organization` renders once, site-wide, in `app/layout.tsx`. Per-page: story pages emit `Article` as the base, upgrading to `Review` (with `reviewRating`) for the review category, `Event` for festival, or `VideoObject` for video, based on which category-specific `meta` fields are actually present — plus `BreadcrumbList` on every page type. Author pages emit `Person`, magazine issues emit `CreativeWork`, products emit `Product` + `Offer`. Every page type also got real canonical URLs and OpenGraph/Twitter Card metadata via `generateMetadata` (previously only the story page had any of this — author/issue/product pages had a bare `title` and nothing else). `app/sitemap.ts` and `app/robots.ts` use Next's built-in `MetadataRoute` convention, dynamically querying published posts/authors/issues/products on every request (not a build-time snapshot) — drafts never appear since every underlying query filters `status = "published"`. The entire `/admin` tree is now `noindex` from one place (`app/admin/layout.tsx`'s `metadata` export, which Next.js's metadata inheritance applies to all 17 child routes) plus `robots.txt`'s `Disallow: /admin` for the crawling half of the same concern.

**Real bug found and fixed during verification, not before:** the `Review` schema's `reviewRating.bestRating` was hardcoded to `10` — wrong for this site, whose own review score convention (confirmed against `components/homepage/CutSection.tsx` and `supabase/seed.sql`'s actual seed values, 88 and 71) is 0-100, not 0-10. Would have shipped a rating that reads as "88 out of 10" to any structured-data consumer. Fixed to `bestRating: 100`; `tests/e2e/story.spec.ts` now asserts the scale directly so this can't silently regress.

**Verification:** could not literally run Google's Rich Results Test (no network egress to an external validator from this environment) or a live Lighthouse SEO pass against a deployed URL, so VERIFY §47 as written is not fully closed — what *is* verified: every JSON-LD block parses as well-formed JSON (piped through Python's `json.loads`, not just eyeballed) and carries the correct `@type` for its page — checked directly against a spotlight post, a below-the-line article, a review, a festival post, a video post, an author page, a product page, and an issue page. `sitemap.xml` and `robots.txt` both resolve and contain the expected content (57 URLs at last count, including the newly-published verification post; `Disallow: /admin` present). `getSiteUrl()`'s fallback chain (`NEXT_PUBLIC_SITE_URL` → Vercel's own URL env vars → the production domain) was exercised in its local/no-override state throughout — confirm the explicit override actually gets set once the real Vercel project exists, since nothing here can verify that env var by itself.
...
**Phase 13 outcome, continued — the "no interim pages" claim above was not actually true yet.** `tests/e2e/interior.spec.ts`'s own `href="#"` sweep test covers `/issues` in its route list, but `components/IssuesGrid.tsx` — the component that page actually renders — was still the pre-CMS, pre-Supabase static demo grid: a hardcoded `COVERS` array (`lib/covers.ts`, comment: "Order and content match issues.html exactly (§7 Phase 1 — zero visual change)") of fictional cover subjects with category filters (Actor/Director/Producer) that don't correspond to anything in the `magazine_issues` schema, and every card linked `href="#"`. This predates the CMS entirely and was never migrated when `/issues/[number]` (the detail page) became real. Fixed: `IssuesGrid.tsx` now takes real `MagazineIssue[]` rows from `getAllIssues()` as a prop, links each card to `/issues/[issue_number]`, shows a "Current Issue" badge from `is_current`, and searches by real issue title instead of filtering by a category concept that doesn't exist in the data model. `lib/covers.ts` deleted (no longer referenced anywhere). Confirmed via `interior.spec.ts`'s existing sweep, which now genuinely passes rather than happening to not catch this.

**Also closed, directly required by §41's own text, not a nice-to-have:** "The admin editor (§45) enforces alt text before an image can be inserted" was not implemented — `TiptapEditor.tsx` inserted every image (drag-and-drop, toolbar upload, and the PDF-to-images batch upload) with `alt: ""`, no prompt, no requirement. Added `promptForAltText()` — blocks insertion entirely if the contributor cancels or leaves it blank, applied to all three insertion paths (the PDF batch path prompts once and auto-suffixes with a page number per image, since prompting per-page across a multi-page PDF would be impractical). `tests/e2e/admin.spec.ts`'s image-upload step updated to handle the resulting `window.prompt()` dialog (Playwright auto-dismisses unhandled dialogs, which would have silently skipped every image insertion in that test).
