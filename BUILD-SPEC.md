...
**Phase 1 outcome — confirmed, and the flip is partial, honestly.** `next/font` fixed the CLS source as predicted: CLS and TBT now pass cleanly on every route, both mobile and desktop, and both are now `error`. Converting cover/logo images to `next/image` (not originally named in the §49 phase table, but required to clear §41's image-delivery mandate and directly relevant to LCP) surfaced and fixed a real bug along the way: the `sharp` optional dependency wasn't installed, so `next/image` was silently serving full-resolution originals unresized — a 900×1323 JPEG for a 384px-wide card thumbnail. Fixing it took `/issues` mobile LCP from 12.7s to ~3.2s.

Desktop clears every §41 threshold, all four metrics, all five routes — fully `error` in `lighthouserc.desktop.json`. Mobile `categories:performance`, CLS, and TBT also clear cleanly and are `error`; mobile LCP does not — three routes (home, issues, submit) still land 200ms–2s over the 2.5s ceiling under Lighthouse's mobile throttling. Per the "commitment, not a hope" principle, mobile LCP stays `warn` rather than being flipped on a metric that doesn't actually pass. This is not the font-loading defect recurring — that's fixed — it's a second, smaller gap (likely `priority`/preload tuning on whichever element actually wins "largest" per route) flagged as follow-up work, not silently absorbed into a blanket flip.
